...
/*   Updated: 2025/03/25 00:09:42 by youbella         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	file_image(t_data *data)
{
	...
	"img/zombie.xpm", &width, &height);
	data->zombie2 = mlx_xpm_file_to_image(data->mlx, \
	"img/zombie2.xpm", &width, &height);
...
void	put_image_to_map(t_data *data, int width, int height)
{
	...
	else if (data->map[height][width] == 'Z')
	{
		if (data->anim_frame / data->anim_speed % 2 == 0)
			put_image(data->zombie, data, width, height);
		else
			put_image(data->zombie2, data, width, height);
	}
	...
	mlx_string_put(data->mlx, data->win, ft_strlen(data->map[0]) * 64 - 50, 15, \
					0x00ff00, ft_itoa(data->xp_count));
}
